...
当前版本支持 **DSH `0.1.7-rc.1`**，通过 `dsh.bundle.patch` 加载。需要原生 `spawn` provider、`userQuestions` 服务、当前平台的问题回答器，以及支持工具调用的模型。
...
语言复用宿主 `locale.preference`：`en` / `en-*` 使用英文，其余按当前中英文支持范围默认中文。`0.1.7-rc.1` 读取 `settings.describe()` 返回的 `locale` 配置条目。每轮开始固定语言，下轮读取新设置；命令目录描述和输入提示在插件加载时确定，更新它们需重新加载插件。未保存偏好或读取失败时默认中文，后端无法获取仅由浏览器自动检测的语言。
