...
export async function runReview(ctx: Context, parent: Agent, target: Target, signal: AbortSignal, timeoutMs?: number, reviewModel?: string): Promise<Outcome> {
  const id = randomUUID();
  const combined = timeoutMs === undefined ? signal : AbortSignal.any([signal, AbortSignal.timeout(timeoutMs)]);
  let run: SubagentRun | undefined;
  let outcome: Outcome = { id, status: 'failed', detail: '审查未完成。' };
  try {
    const cwd = parent.session.header.cwd;
    if (!cwd) throw new Error('会话没有有效 cwd。');
    const prompt = await reviewPrompt(target, cwd, combined);
    const provider = ctx.subagents.getProvider('spawn');
    if (!provider || provider.inheritsParentContext || !provider.capabilities.persona) throw new Error('需要独立上下文的原生 spawn provider。');
    const scope = {};
    ...
    try {
      run = await creationScope.run(scope, () => ctx.subagents.start('spawn', {
        parent, label: '代码审查', signal: combined, persona: REVIEW_PROMPT,
        ...(reviewModel ? { agentOptions: { model: reviewModel } } : {}),
        prompt: [{ type: 'text', text: prompt }],
      }));
    } finally { stopSetup(); }
    if (!configured || !run.localAgent) throw new Error('原生审查 Agent 未完成配置。');
    const result = await run.result;
    combined.throwIfAborted();
    if (result.stopReason !== 'completed') throw new Error(`原生子 Agent 未完成：${result.stopReason}`);
    const raw = result.output.filter(b => b.type === 'text').map(b => b.text).join('');
    if (!raw.trim()) throw new Error('审查 Agent 未返回报告。');
    ...
    if (run) {
      ...
      catch (error) { outcome = { ...outcome, status: 'failed', detail: `Agent 清理失败：${String(error)}` }; }
    }
  }
  return outcome;
}
